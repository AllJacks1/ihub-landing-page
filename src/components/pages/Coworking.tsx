...
export default function CoworkingPage() {
  return (
    <main className="min-h-screen bg-stone-50">
      ...
      <section
        id="passes"
        className="bg-stone-50 px-4 py-16 sm:px-6 sm:py-20 md:py-24"
      >
        <div className="mx-auto max-w-6xl">
          {/* Header */}
          <div className="mb-12 text-center sm:mb-16">
            <Badge
              variant="outline"
              className="mb-4 border-stone-200 bg-white px-3.5 py-1 text-[11px] font-bold tracking-widest text-stone-500"
            >
              MEMBERSHIP PLANS
            </Badge>
            <h2 className="mb-3 font-serif text-3xl font-semibold tracking-tight text-stone-900 sm:text-4xl md:text-5xl">
              Choose Your Plan
            </h2>
            <p className="mx-auto max-w-lg text-base text-stone-500 sm:text-lg">
              ...
              <Card
                ...
                className={`relative flex flex-col overflow-hidden transition-all duration-300 ${
                  plan.variant === "featured"
                    ? "z-10 border-[#F36509]/40 bg-white shadow-xl shadow-orange-500/10 ring-1 ring-[#F36509]/20 md:-translate-y-2 hover:-translate-y-3"
                    : "border-stone-200 bg-white shadow-sm hover:-translate-y-1.5 hover:shadow-md"
                }`}
              >
                {/* Featured badge – centered top */}
                {plan.badge && (
                  <div className="absolute top-3 left-1/2 z-20 -translate-x-1/2">
                    <div className="flex items-center gap-1.5 rounded-full bg-[#F36509] px-3.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white shadow-md">
                ...
                <CardContent className="flex flex-1 flex-col p-6 pt-8 sm:p-8">
                  {/* Tag */}
                  <div className="mb-2 font-mono text-[10px] font-bold tracking-widest text-[#F36509] sm:text-[11px]">
                    {plan.tag}
                  </div>

                  {/* Name */}
                  <h3 className="mb-4 font-serif text-2xl font-semibold tracking-tight text-stone-900 sm:text-[1.65rem]">
                    {plan.name}
                  </h3>

                  {/* Price */}
                  <div className="mb-1 flex items-baseline gap-1.5">
                    <span className="text-3xl font-bold tracking-tight text-stone-900 sm:text-4xl">
                      {plan.price}
                    </span>
                  </div>
                  <div className="mb-5 text-sm text-stone-400">
                    {plan.period}
                  </div>
                  ...
                  {/* Study tiers (if any) */}
                  {plan.tiers && (
                    <div className="mb-6 space-y-2.5 rounded-xl border border-stone-100 bg-stone-50/80 p-4">
                      {plan.tiers.map((tier) => (
                        <div
                          key={tier.hours}
                          className="flex items-center justify-between text-sm"
                        >
                          <span className="text-stone-600">{tier.hours}</span>
                  ...
                  <Button
                    className={`h-12 w-full rounded-full text-sm font-semibold transition-all ${
                      plan.variant === "featured"
                        ? "bg-[#F36509] text-white shadow-md shadow-orange-500/25 hover:bg-[#e05a00]"
                        : plan.variant === "highlighted"
                          ? "border-2 border-[#F36509] bg-transparent text-[#F36509] hover:bg-[#F36509] hover:text-white"
                          : "border-2 border-stone-200 bg-white text-stone-800 hover:border-stone-900 hover:bg-stone-900 hover:text-white"
                    }`}
